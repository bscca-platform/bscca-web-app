...
export async function generateMetadata({ params }: PlayerPageProps): Promise<Metadata> {
    ...
    try {
        const player = await api.getPlayerBySlug(slug);
        if (!player) return { title: "Player Not Found" };

        return {
            title: `${player.name} Player Profile - BSCCA | Beltala Cricket`,
            description: `${player.name} (${player.role}) official profile. View career stats, strike rate, and achievements on the official BSCCA platform.`,
            openGraph: {
                title: `${player.name} Profile | BSCCA Cricket Elite`,
                description: `Professional stats and bio for ${player.name}.`,
                images: player.image ? [{ url: player.image }] : [],
            },
        };
    } catch (err) {
...
export default async function PlayerPage({ params }: PlayerPageProps) {
    ...
    let player;
    try {
        player = await api.getPlayerBySlug(slug);
    } catch (err) {
        notFound();
    }
    ...
    const jsonLd = {
        ...
        "@type": "Person",
        "name": player.name,
        "jobTitle": player.role || "Cricketer",
        "affiliation": {
            ...
            <section className="relative rounded-3xl overflow-hidden bg-white border-b border-border py-12 sm:py-20">
                <div className="max-w-7xl mx-auto px-6 relative z-10 mb-6">
                    <Breadcrumbs items={[
                        ...
                        { label: player.name, href: `/players/${teamSlug}/${slug}` }
                    ]} />
                </div>
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
                    ...
                    <div className="w-64 h-64 md:w-80 md:h-96 bg-muted rounded-3xl overflow-hidden shadow-xl border border-border/50">
                        {player.image ? (
                            <img src={player.image} alt={`${player.name} - BSCCA Player Profile`} className="w-full h-full object-cover" />
                        ) : (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    <div className="lg:col-span-8 space-y-10">
                        ...
                        <div className="space-y-4">
                            <h2 className="text-2xl font-semibold tracking-tight text-foreground">About {player.name}</h2>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="space-y-3">
                            <h3 className="text-base font-semibold text-foreground">Social Presence</h3>
                        ...
                        <div className="space-y-3">
                            <h3 className="text-base font-semibold text-foreground">Recent Activity</h3>
                                <div className="p-5 bg-white border border-border/60 rounded-2xl hover:border-accent/30 transition-all group">
                                    <span className="text-[10px] text-accent font-medium uppercase tracking-wide">Field Presence</span>
                                    <p className="text-sm font-medium text-foreground mt-1 leading-relaxed">
                                        {player.name} displayed exceptional performance in recent BSCCA match fixtures.
                                    </p>
                                    <div className="mt-3 flex items-center justify-between text-[10px] text-muted-foreground">
                                        <span>2026 Season</span>
                                        <Badge className="bg-muted text-muted-foreground text-[9px] border-none px-2 py-0.5 rounded-md">Verified Profile</Badge>
                                    </div>
                                </div>
                        </div>
                    </div>
