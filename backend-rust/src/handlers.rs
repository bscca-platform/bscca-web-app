...
pub async fn finish_match(
    ...
    while let Ok(Some(row)) = bowling_rows.next().await {
        ...
        let _ = state.db.client.execute(
            "UPDATE players SET wickets = wickets + ?, runs_conceded = runs_conceded + ?, overs_bowled = ? WHERE id = ?", // overs_bowled update logic is complex, simplified for now
            libsql::params![p_wickets, p_runs, "updated", p_id]
        ).await;
    }
